...
g_update <- function(Y, D, theta, g.current) {
  ...
  kde.u.hat <- ks::kde(u.hat)
  kde.est <- predict(kde.u.hat, x = u.hat)
  g.hat <- log(sqrt(2*pi)*kde.est) + (u.hat^2)/2
  g.curr <- init_g(g.hat, u.hat, g.current$a0, g.current$a1, g.current$phi)
  return(g.curr)
  # u.hat<- sqrt(D)*(Y - theta)
...
make_mhingibbs_sampler <- function(X, Y, D, var_gamma_a=0.01, var_gamma_b=0.01, 
  ...
  mh_in_gibbs_sampler <- function(params.init, n.samples, n.thin, mh.scale.thetas) {
