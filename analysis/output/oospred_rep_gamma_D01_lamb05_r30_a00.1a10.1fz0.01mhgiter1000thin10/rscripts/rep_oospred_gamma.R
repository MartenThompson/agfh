...
run_it <- function(dat_maker, M.tr, reps, slug.data.name, kern.a0, kern.a1, kern.fuzz, mhg.iter, thin) {
  ...
  file.copy('./analysis/rep_oospred_gamma.R', paste0(save_slug, 'rscripts/rep_oospred_gamma.R'))
...
set.seed(rng.seed)
D <- rep(1/10, M.train+M.test)
data_maker <- function() {
  split_traintest(gamma_err_gen(M.train+M.test, p, D, 0.5, shape = 1/2, rate = 10), M.train, M.test)
}
run.name <- 'oospred_rep_gamma_D01_lamb05'
