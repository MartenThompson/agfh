...
run_it <- function(dat_maker, M.tr, reps, slug.data.name, kern.a0, kern.a1, kern.fuzz, mhg.iter, thin) {
  ...
  file.copy('./analysis/rep_oospred_nullgen.R', paste0(save_slug, 'rscripts/rep_oospred_nullgen.R'))
...
data_maker <- function() {
  split_traintest(null_gen(M.train+M.test, p, D, 0.5), M.train, M.test)
}
run.name <- 'oospred_rep_nullgen_D01_lamb05'
