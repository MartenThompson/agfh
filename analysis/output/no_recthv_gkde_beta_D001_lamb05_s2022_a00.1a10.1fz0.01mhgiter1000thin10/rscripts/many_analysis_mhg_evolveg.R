...
set.seed(rng.seed)
D <- rep(1/100, M)
dat <- null_gen(M=M, p=p, D=D, lambda=1/2)
run.name <- 'no_recthv_gkde_nullgen_D001_lamb05'
...
run.name <- 'no_rechthv_gkde_gamma_D001_lamb05'
run_it(rng.seed, run.name, dat, kernel.a0, kernel.a1, kernel.fuzz, mhg.iter, mhg.thin)


set.seed(rng.seed)
D <- rep(1/100, M)
dat <- beta_err_gen(M, p, D, 1/2, 1/8, 1/8)
run.name <- 'no_recthv_gkde_beta_D001_lamb05'
run_it(rng.seed, run.name, dat, kernel.a0, kernel.a1, kernel.fuzz, mhg.iter, mhg.thin)
...
run.name <- 'no_recthv_gkde_betaassym_D001_lamb05'
run_it(rng.seed, run.name, dat, kernel.a0, kernel.a1, kernel.fuzz, mhg.iter, mhg.thin)
