...
mh.scale.thetas <- 5e6 # 1e8 for 55k - 450k
n.mcmc <- 1e2
n.thin <- 1e2
...
save.slug <- paste0('analysis/output/', 'saipe_county/', 'mh',mh.scale.thetas,'nmcmc', n.mcmc, 'thin', n.thin, '/')
dir.create(file.path(save.slug))
dir.create(file.path(save.slug, 'robjects/', fsep=''))
dir.create(file.path(save.slug, 'rscripts/', fsep=''))

file.copy('./analysis/analysis_saipe_county.R', paste0(save.slug, 'rscripts/', 'analysis_saipe_county.R'))

#~~~~~~~~~~~~~~~~~~~~~~~~~~~~~#
#### Data Read ####
#~~~~~~~~~~~~~~~~~~~~~~~~~~~~~#

dat.raw <- readxl::read_xls('./data/est19all.xls', sheet = 1)
n.rows <- length(dat.raw$...2)
fips.codes <- dat.raw$...2[6:n.rows]
keep <- (fips.codes!='000') & (dat.raw$...4[6:n.rows] != 'Kalawao County')# exclude state level entries
state.names <- dat.raw$...3[6:n.rows][keep] 
county.names <- dat.raw$...4[6:n.rows][keep]
stcnty.names <- paste0(state.names, '_', county.names)
poverty.90l <- as.numeric(dat.raw$...6[6:n.rows][keep])
poverty.50 <- as.numeric(dat.raw$...5[6:n.rows][keep])
poverty.90h <- as.numeric(dat.raw$...7[6:n.rows][keep])
summary(poverty.50)


#income.90l <- as.numeric(dat.raw$...23[5:n.rows])
income.50 <- as.numeric(dat.raw$...23[6:n.rows][keep])
summary(income.50)
#income.90h <- as.numeric(dat.raw$...24[5:n.rows])



subset <- sample(1:sum(keep), 200)
Y <- poverty.50[subset]
# 90% CI so 0.05 - 0.95 percent
D <- ((2*1.645)/(poverty.90h - poverty.90l))^2
D <- D[subset]
X <- as.matrix(income.50, ncol=1)
X <- matrix(X[subset,], ncol=1)
