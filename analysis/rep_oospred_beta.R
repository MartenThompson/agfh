...
run_it <- function(dat_maker, M.tr, reps, slug.data.name, kern.a0, kern.a1, kern.fuzz, mhg.iter, thin) {
  ...
  file.copy('./analysis/rep_oospred_beta.R', paste0(save_slug, 'rscripts/rep_oospred_beta.R'))
...
data_maker <- function() {
  split_traintest(beta_err_gen(M.train+M.test, p, D, 0.5, 1/8, 1/8), M.train, M.test)
}
run.name <- 'oospred_rep_beta_D01_lamb05'
